        ...
        if (categories.length === 0) {
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <button class="btn btn-primary" data-action="initDefaultCategories">Standardkategorien erstellen</button>
                </div>
            `;
            return;
        }
    ...
    for (const cat of cats) {
        ...
        html += `
            <div class="category-item ${level > 0 ? 'subcategory' : ''}" style="padding-left: ${16 + paddingLeft}px;" data-id="${cat.id}">
                ...
                <div class="actions">
                    <button class="btn btn-sm btn-icon" data-action="editCategory" data-id="${cat.id}" title="Bearbeiten">
                    ...
                    ` : ''}
                    <button class="btn btn-sm btn-icon" data-action="deleteCategory" data-id="${cat.id}" title="Löschen">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
