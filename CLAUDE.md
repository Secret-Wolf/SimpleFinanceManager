...
- 🔴 **OFFEN/HIGH — Login-Rate-Limit von außen umgehbar via `X-Real-IP`-Spoofing** (rotierender Header → jeder Wert eigener slowapi-Bucket; live 12/12 ohne 429) → unbegrenzte Passwort-Brute-Force + **Audit-Log-IPs spoofbar**. Ursache liegt in der NPM→uvicorn-Kette (die App liest `X-Real-IP` nicht selbst, nur `request.client.host`). **Fix-Versuch fehlgeschlagen:** `proxy_set_header X-Real-IP/X-Forwarded-For $remote_addr` im NPM-**Advanced-Tab (server-Ebene)** wirkt nicht, weil NPMs `location`-Block eigene `proxy_set_header` hat und nginx server-level Header dann **ersetzt statt merged**. Korrekter Fix ausstehend: Override auf **location-Ebene** (NPM „Custom Locations") + `FORWARDED_ALLOW_IPS` exakt auf `192.168.178.5` (nicht `*`); optional app-seitige Client-IP-Normalisierung als Defense-in-Depth. Verifikation: `data/logs/audit.log` zeigt bei Spoof-Logins die Fake-IPs.
