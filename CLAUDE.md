...
- **Image with ALL features above (incl. Security-Hardening + PWA/Mobile-Rework) built & pushed 2026-07-02** (`192.168.178.30:5000/finanzmanager:latest`, digest `920bb7ff…`; ersetzt das frühere `70a66589…` vom selben Tag). Server deploy: `docker compose pull && docker compose up -d` **against the existing production DB** (802 transactions, 376 categorized, 80 categories, 19 rules, 2 users, account "Giro Young" DE36…8800). Migrations 16–18 are additive; 16+17 verified against a copy of an existing DB, 18 covered by the test suite. Existing logins survive the deploy (tokens without `ver` count as version 0).
